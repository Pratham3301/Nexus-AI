...
class CalendarAgent(LlmAgent):
    ...
    async def _get_google_service(self, user_id: str):
        # Always try demo_user as fallback — the AI sometimes passes wrong user_id
        for uid in [user_id, 'demo_user']:
            db = get_db()
            ...
            if user_doc.exists:
                ...
                if creds_data:
                    log.info('calendar.credentials.found', user_id=uid)
                    creds = Credentials(
                    ...
                    if not creds.valid and creds.refresh_token:
                        try:
                            ...
                            log.info('calendar.token.refreshed', user_id=uid)
                        except Exception as e:
                            log.warning('calendar.token.refresh_failed', error=str(e))
                    service = build('calendar', 'v3', credentials=creds)
                    return service
        log.warning('calendar.no_credentials', user_id=user_id)
        return None
    ...
    async def get_upcoming_events(self, user_id: str, days_ahead: int = 7) -> list:
        ...
        if service:
            ...
            for e in events:
                event_copy = dict(e)
                # Convert ALL Firestore special types (DatetimeWithNanoseconds, etc.) to strings
                for key, val in event_copy.items():
                    if hasattr(val, 'isoformat'):
                        event_copy[key] = val.isoformat()
                    elif not isinstance(val, (str, int, float, bool, type(None), list, dict)):
                        event_copy[key] = str(val)
                # Ensure start_time is a string for comparison
                st = str(event_copy.get('start_time', ''))
