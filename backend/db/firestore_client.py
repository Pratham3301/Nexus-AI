...
def _serialize(data: dict) -> dict:
    """Convert Firestore special types (DatetimeWithNanoseconds, etc.) to JSON-safe values."""
    result = {}
    for key, val in data.items():
        if hasattr(val, 'isoformat'):        # datetime / DatetimeWithNanoseconds
...
class FirestoreClient:
    ...
    async def get(self, collection: str, doc_id: str) -> Optional[dict]:
        ...
        if doc.exists:
            return _serialize({"id": doc.id, **doc.to_dict()})
    ...
    async def query(
        ...
        ref = ref.limit(limit)
        docs = await ref.get()
        result = [_serialize({"id": d.id, **d.to_dict()}) for d in docs]
